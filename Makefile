...
COVERAGE_EXE ?= ./node_modules/.bin/coffeeCoverage
COVERAGE_ARGS ?= --initfile $(LIB_COV)/coffee-coverage-init.js
MOCHA_COV_ARGS  ?= --require $(LIB_COV)/coffee-coverage-init.js --globals "_\$$jscoverage" --compilers coffee:coffee-script/register -R html-cov
...
clean-coverage:
	rm -r $(RM_DASH_I) $(COVERAGE_TMP_DIR)
	rm -r $(RM_DASH_I) $(LIB_COV)
	rm $(RM_DASH_I) $(COVERAGE_REPORT)
...
coverage:
	...
	coffeeCoverage --initfile $(LIB_COV)/coffee-coverage-init.js $(COVERAGE_TMP_DIR) $(LIB_COV)
